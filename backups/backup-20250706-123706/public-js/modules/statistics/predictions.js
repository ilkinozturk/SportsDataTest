...
(function(global) {
  ...
  const PredictionsStatistics = {
    ...
    predictGoals(teamData, opponentData, venue) {
      ...
      // Get goals for/against averages
    ...
    predictResult(teamData, opponentData, venue) {
      ...
      return {
        ...
        confidence: this.calculateConfidence(stats, ['winPercentage', 'drawPercentage', 'lossPercentage'])
      };
    },
    ...
    predictCorners(teamData, opponentData, venue) {
      ...
      return {
        ...
        confidence: this.calculateConfidence(stats, ['cornersForPerMatch', 'cornersAgainstPerMatch'])
      };
    },
    ...
    predictBTTS(teamData, opponentData, venue) {
      ...
      const goalsFor = this.getStatValue(stats, 'goalsForPerMatch', venue === 'home' ? 'home' : venue === 'away' ? 'away' : '') || 1.5;
      const goalsAgainst = this.getStatValue(stats, 'goalsAgainstPerMatch', venue === 'home' ? 'home' : venue === 'away' ? 'away' : '') || 1.0;
      ...
      return {
        ...
        confidence: this.calculateConfidence(stats, ['bttsPercentage', 'goalsForPerMatch', 'goalsAgainstPerMatch'])
      };
    },
    ...
    getStatValue(stats, baseName, venuePrefix) {
      if (venuePrefix) {
        ...
        if (stats[venueKey] !== undefined) return stats[venueKey];
        
        const suffixKey = `${baseName}_${venuePrefix}`;
        if (stats[suffixKey] !== undefined) return stats[suffixKey];
      }
