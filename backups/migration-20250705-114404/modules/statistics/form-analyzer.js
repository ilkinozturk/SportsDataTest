...
(function(global) {
    ...
    class FormAnalyzer {
        constructor() {
            ...
            if (!this.baseStats) {
                console.error('[FormAnalyzer] BaseStatistics module is required');
            }
        ...
        analyzeMomentum(matches, teamId) {
            ...
            const recentScore = this.calculateMomentumScore(recentMatches, teamId) * this.momentumWeights.recent;
            const mediumScore = this.calculateMomentumScore(mediumMatches, teamId) * this.momentumWeights.medium;
            const olderScore = this.calculateMomentumScore(olderMatches, teamId) * this.momentumWeights.older;
        ...
        analyzeVenueForm(matches, teamId) {
            const homeMatches = matches.filter(match => 
                String(match.homeTeamId || match.home_team_id) === String(teamId)
            );
            const awayMatches = matches.filter(match => 
                String(match.awayTeamId || match.away_team_id) === String(teamId)
            );
            ...
            if (options.venue && options.venue !== 'overall') {
                if (options.venue === 'home') {
                    filtered = filtered.filter(match => 
                        String(match.homeTeamId || match.home_team_id) === String(options.teamId)
                    );
                } else if (options.venue === 'away') {
                    filtered = filtered.filter(match => 
                        String(match.awayTeamId || match.away_team_id) === String(options.teamId)
                    );
                }
            }
        ...
        calculateResultVariance(results) {
            ...
            return Math.sqrt(
                (Math.pow(counts.W - expected, 2) + 
                 ...
                 Math.pow(counts.L - expected, 2)) / 3
            );
        }
        ...
        compareRecentVsHistorical(matches, teamId) {
            ...
            return {
                ...
                comparison: recentForm.pointsPerGame > historicalForm.pointsPerGame ? 'improving' : 
                           recentForm.pointsPerGame < historicalForm.pointsPerGame ? 'declining' : 'stable'
            };
        }
