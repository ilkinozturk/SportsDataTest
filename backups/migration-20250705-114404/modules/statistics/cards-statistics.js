    ...
    class CardsStatistics {
        ...
        calculateCardStatistics(matches, options = {}) {
            ...
            const stats = {
                ...
                // Card types
                yellowCards: this.calculateCardsByType(filteredMatches, this.cardTypes.YELLOW, options.teamId),
        ...
        filterMatches(matches, options = {}) {
            ...
            if (options.referee) {
                filtered = filtered.filter(match => 
                    match.referee === options.referee ||
                    match.refereeId === options.referee
                );
            }
        ...
        getMatchCards(match, side) {
            ...
            const yellowCards = parseInt(
                match[`${side}YellowCards`] || 
                match[`${side}_yellow_cards`] || 
                match[`${side}Yellow`] || 
                0
            );
        ...
        calculateDisciplinaryRecord(matches, teamId) {
            ...
            return {
                ...
                avgCardsPerMatch: matches.length > 0 
