    ...
    class CornersStatistics {
        ...
        filterMatches(matches, options = {}) {
            ...
            if (options.minCorners || options.maxCorners) {
                filtered = filtered.filter(match => {
                    const totalCorners = this.getMatchCorners(match, 'home') + this.getMatchCorners(match, 'away');
        ...
        calculateOverUnder(matches) {
            ...
            this.overUnderThresholds.forEach(threshold => {
                ...
                stats[underKey] = {
                    ...
                    percentage: this.baseStats.percentage(matches.length - overMatches.length, matches.length)
                };
            });
            ...
            matches.forEach(match => {
                if (match.cornerTimings && Array.isArray(match.cornerTimings)) {
                    match.cornerTimings.forEach(corner => {
                        ...
                        const interval = this.timeIntervals.find(int => 
                            minute >= int.start && minute <= int.end
                        );
        ...
        calculateCornerPatterns(matches, teamId) {
            ...
            matches.forEach(match => {
                ...
                if (teamCorners > 0) {
                    ...
                    if (match.cornerTimings && match.cornerTimings.length > 0) {
                        ...
                        if (teamCornerTimings.length > 0) {
                            ...
                            lastCornerMinutes.push(parseInt(teamCornerTimings[teamCornerTimings.length - 1].minute || 90));
                        }
                    }
                } else {
                    currentNoCornerStreak++;
                    currentCornerStreak = 0;
                    patterns.highestNoCornerStreak = Math.max(patterns.highestNoCornerStreak, currentNoCornerStreak);
                }
                ...
                // Most corners half
                const firstHalfTeamCorners = parseInt(isHome ? 
                    (match.firstHalfHomeCorners || 0) : (match.firstHalfAwayCorners || 0));
            ...
            patterns.cornerDominance = this.baseStats.percentage(patterns.cornerDominance, matches.length);
        ...
        calculateEarlyCorners(matches) {
            ...
            matches.forEach(match => {
                ...
                if (match.cornerTimings && Array.isArray(match.cornerTimings)) {
                    const early = match.cornerTimings.filter(corner => 
                        parseInt(corner.minute || corner.time) <= 30
                    );
                    if (early.length > 0) {
        ...
        calculateLateCorners(matches) {
            ...
            matches.forEach(match => {
                ...
                if (match.cornerTimings && Array.isArray(match.cornerTimings)) {
                    const late = match.cornerTimings.filter(corner => 
                        parseInt(corner.minute || corner.time) >= 75
                    );
                    if (late.length > 0) {
            ...
            matches.forEach(match => {
                if (match.cornerTimings && Array.isArray(match.cornerTimings)) {
                    ...
                    const teamCornerTimings = match.cornerTimings
                        .filter(corner => (isHome && corner.team === 'home') || (!isHome && corner.team === 'away'))
                    ...
                    for (let i = 0; i < teamCornerTimings.length; i++) {
                        ...
                        if (i > 0) {
                            const timeDiff = (teamCornerTimings[i].minute || teamCornerTimings[i].time) - 
                        ...
                        if (i < match.cornerTimings.length - 1) {
                            ...
                            if ((isHome && nextCorner.team === 'away') || (!isHome && nextCorner.team === 'home')) {
                                consecutive = 0;
                            }
                        }
                    }
        ...
        calculateCornerForm(matches, teamId) {
            ...
            return {
                ...
                avgCornersFor: last5.length > 0 
                    ? this.baseStats.divide(this.calculateCornersFor(last5, teamId), last5.length)
                    : 0,
                avgCornersAgainst: last5.length > 0
                    ? this.baseStats.divide(this.calculateCornersAgainst(last5, teamId), last5.length)
                    : 0,
                totalCorners: last5.reduce((sum, match) => 
                    sum + this.getMatchCorners(match, 'home') + this.getMatchCorners(match, 'away'), 0
                ),
                over9_5: last5.filter(match => 
                    this.getMatchCorners(match, 'home') + this.getMatchCorners(match, 'away') > 9.5
                ).length
            };
        }
        ...
        calculateCornerEfficiency(matches, teamId) {
            ...
            matches.forEach(match => {
                ...
                if (match.goalDetails && Array.isArray(match.goalDetails)) {
                    const teamGoalsFromCorners = match.goalDetails.filter(goal => {
                        const isTeamGoal = (isHome && goal.team === 'home') || (!isHome && goal.team === 'away');
            ...
            return {
                goalsFromCorners,
                totalCorners,
                efficiency: totalCorners > 0 ? this.baseStats.percentage(goalsFromCorners, totalCorners) : 0,
                cornersPerGoal: goalsFromCorners > 0 ? this.baseStats.divide(totalCorners, goalsFromCorners) : 0
            };
        }
