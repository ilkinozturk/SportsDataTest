    ...
    class XGStatistics {
        constructor() {
            ...
            this.shotQuality = {
                ...
                MEDIUM: { min: 0.15, max: 0.30, label: 'Medium' },
                HIGH: { min: 0.30, max: 0.50, label: 'High' },
                VERY_HIGH: { min: 0.50, max: 1.0, label: 'Very High' }
            };
        ...
        filterMatches(matches, options = {}) {
            ...
            if (options.competition) {
                filtered = filtered.filter(match => 
                    match.competition === options.competition ||
                    match.competitionId === options.competition
                );
            }
            ...
            if (options.xgOnly) {
                filtered = filtered.filter(match => 
                    match.xG || match.expectedGoals || match.xg
                );
            }
        ...
        calculatePerformanceVsXG(matches, teamId) {
            ...
            matches.forEach(match => {
                ...
                const hasXGData = (match.homeXG !== undefined && match.awayXG !== undefined) ||
                                  match.xG || match.expectedGoals || match.xg;
                if (!hasXGData) return;
            ...
            return {
                ...
                defensiveEfficiency: xGAgainst > 0 ? this.baseStats.percentage(actualGoalsAgainst, xGAgainst) : 0,
        ...
        calculateXGOverUnder(matches) {
            ...
            this.xgThresholds.forEach(threshold => {
                ...
                stats[key] = {
                    ...
                    percentage: this.baseStats.percentage(overMatches.length, this.countMatchesWithXG(matches))
                };
            });
        ...
        calculateTeamXGOver(matches, teamId) {
            ...
            thresholds.forEach(threshold => {
                ...
                const overMatches = matches.filter(match => {
                    ...
                    const isHome = String(match.homeTeamId || match.home_team_id) === String(teamId);
                    const teamXG = isHome 
                        ? parseFloat(xgData.home || 0)
                        : parseFloat(xgData.away || 0);
        ...
        analyzeShotQuality(matches, teamId) {
            ...
            matches.forEach(match => {
                ...
                const isHome = String(match.homeTeamId || match.home_team_id) === String(teamId);
                const teamShots = match.shots.filter(shot => 
                    (isHome && shot.team === 'home') || (!isHome && shot.team === 'away')
                );
        ...
        calculateXGBySituation(matches, teamId) {
            ...
            Object.keys(situations).forEach(situation => {
                ...
                data.conversionRate = data.shots > 0 
                    ? this.baseStats.percentage(data.goals, data.shots)
                    : 0;
                data.xgPerShot = data.shots > 0
                    ? this.baseStats.divide(data.xg, data.shots)
                    : 0;
            });
        ...
        calculateXGTiming(matches, teamId) {
            ...
            Object.keys(timing).forEach(interval => {
                ...
                data.xg = this.baseStats.round(data.xg, 2);
                data.avgXG = data.shots > 0 
                    ? this.baseStats.divide(data.xg, data.shots, 3)
                    : 0;
            });
        ...
        calculateXGChain(matches, teamId) {
            ...
            return {
                ...
                average: matchesWithData > 0 
                    ? this.baseStats.divide(totalXGChain, matchesWithData)
                    : 0,
        ...
        calculateNonPenaltyXG(matches, teamId) {
            ...
            matches.forEach(match => {
                ...
                if (match.npxg) {
                    const npxg = isHome 
                        ? parseFloat(match.npxg.home || 0)
                        : parseFloat(match.npxg.away || 0);
                    totalNPxG += npxg;
                } else {
                    // Calculate from total xG minus penalty xG
                    const totalXG = isHome 
                        ? parseFloat(xgData.home || 0)
                        : parseFloat(xgData.away || 0);
                    const penaltyXG = match.penaltyXG ? (isHome 
                        ? parseFloat(match.penaltyXG.home || 0)
                        : parseFloat(match.penaltyXG.away || 0)) : 0;
            ...
            return {
                ...
                avgNPxG: matchesWithData > 0 
                    ? this.baseStats.divide(totalNPxG, matchesWithData)
                    : 0,
                avgPenaltyXG: matchesWithData > 0 
                    ? this.baseStats.divide(totalPenaltyXG, matchesWithData)
                    : 0,
                matches: matchesWithData
            };
        }
        ...
        calculateBigChances(matches, teamId) {
            ...
            return {
                ...
                conversionRateFor: bigChancesFor > 0 
                    ? this.baseStats.percentage(bigChancesScoredFor, bigChancesFor)
                    : 0,
                conversionRateAgainst: bigChancesAgainst > 0 
                    ? this.baseStats.percentage(bigChancesScoredAgainst, bigChancesAgainst)
                    : 0,
                avgBigChancesFor: matchesWithData > 0 
                    ? this.baseStats.divide(bigChancesFor, matchesWithData)
                    : 0,
                avgBigChancesAgainst: matchesWithData > 0 
                    ? this.baseStats.divide(bigChancesAgainst, matchesWithData)
                    : 0,
                matches: matchesWithData
            };
        }
        ...
        getEmptyStatistics() {
            return {
                ...
                xgTrends: { xgForTrend: [], xgAgainstTrend: [], xgDiffTrend: [], rollingAverage: { xgFor: [], xgAgainst: [] } },
                highXGMatches: [],
                ...
                bigChances: { bigChancesFor: 0, bigChancesAgainst: 0, bigChancesScoredFor: 0, bigChancesScoredAgainst: 0, conversionRateFor: 0, conversionRateAgainst: 0, avgBigChancesFor: 0, avgBigChancesAgainst: 0, matches: 0 }
            };
        }
