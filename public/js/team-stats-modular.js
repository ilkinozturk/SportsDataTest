  ...
  const TeamStatsModular = {
      ...
      if (global.TeamStatsTeamService) {
        try {
          ...
          if (teamData && (teamData.statistics || teamData.stats)) {
            ...
            if (
              global.TeamStatsGoalsDisplay &&
              global.TeamStatsGoalsDisplay.updateGoalsStatistics
            ) {
    ...
    updateDisplaysForTab(tab, stats) {
      ...
      displays.forEach(displayName => {
        const moduleKey = `TeamStats${displayName
          .split('-')
        ...
        if (display && display.renderSection) {
          const container =
            document.querySelector(`.${displayName}-container`) ||
      ...
      if (
        tab === 'goals' &&
        global.TeamStatsGoalsDisplay &&
        global.TeamStatsGoalsDisplay.updateConcededSection
      ) {
        global.TeamStatsGoalsDisplay.updateConcededSection(stats, 'overall');
      }
    },
    ...
    populateTeamInfo(teamData) {
      ...
      if (teamNameEl) {
        const teamName =
          teamData.teamInfo?.name || teamData.teamName || teamData.name || 'Unknown Team';
      ...
      if (teamLogoEl) {
        const logoUrl =
          teamData.teamInfo?.image || teamData.teamInfo?.logo || teamData.teamLogo || teamData.logo;
        ...
        if (logoUrl && logoUrl.length > 0 && !teamLogoEl.querySelector('img')) {
          ...
          const firstLetter = (teamData.teamInfo?.name || teamData.teamName || teamData.name || 'T')
            .charAt(0)
            .toUpperCase();
      ...
      if (formTextEl) {
        const wins = (stats.recentForm || '')
          .slice(-5)
          .split('')
          .filter(l => l.toLowerCase() === 'w').length;
      ...
      if (penaltiesConcededEl) {
        penaltiesConcededEl.textContent =
          stats.penaltiesConceded !== undefined ? stats.penaltiesConceded : '0';
      }
    ...
    updateSectionStatistics(section, stats, filter) {
      ...
      statElements.forEach(element => {
        ...
        if (filter !== 'overall') {
          ...
          if (filter === 'home') {
            fieldName =
              stats[`home${statKey.charAt(0).toUpperCase() + statKey.slice(1)}`] !== undefined
                ? `home${statKey.charAt(0).toUpperCase() + statKey.slice(1)}`
                : `${statKey}_home`;
          } else if (filter === 'away') {
            fieldName =
              stats[`away${statKey.charAt(0).toUpperCase() + statKey.slice(1)}`] !== undefined
    ...
    populateStatistics(stats) {
      ...
      if (
        global.TeamStatsCornersDisplay &&
        global.TeamStatsCornersDisplay.updateCornersStatistics
      ) {
        ...
        if (filterBtn) {
          ...
          if (filterValue) {
            ...
            if (section) {
              ...
              const sectionButtons = section.querySelectorAll(
                `.section-filter[data-filter-type="${filterType}"]`
              );
    ...
    updateStatisticsForFilterType(stats, filterType, filter) {
      ...
      switch (filterType) {
        ...
        case 'matchCards':
          ...
          if (
            global.TeamStatsCardsDisplay &&
            global.TeamStatsCardsDisplay.updateMatchCardsSection
          ) {
        ...
        case 'corners':
          ...
          if (
            global.TeamStatsCornersDisplay &&
            global.TeamStatsCornersDisplay.updateMatchCornersSection
          ) {
        ...
        case 'teamCorners':
          ...
          if (
            global.TeamStatsCornersDisplay &&
            global.TeamStatsCornersDisplay.updateTeamCornersSection
          ) {
    ...
    updateCardsStatistics(stats, filter) {
      ...
      this.updateElement(
        'cardsHighest',
        stats[`cardsHighest${suffix}`] || stats.cardsHighest_overall || 0
      );
      this.updateElement(
        'cardsLowest',
        stats[`cardsLowest${suffix}`] || stats.cardsLowest_overall || 0
      );

      // Cards Over statistics
      this.updateElement(
        'cardsOver05',
        (stats[`cardsOver05${suffix}`] || stats.over05Cards || 0) + '%'
      );
      this.updateElement(
        'cardsOver15',
        (stats[`cardsOver15${suffix}`] || stats.over15Cards || 0) + '%'
      );
      this.updateElement(
        'cardsOver25',
        (stats[`cardsOver25${suffix}`] || stats.over25Cards || 0) + '%'
      );
      this.updateElement(
        'cardsOver35',
        (stats[`cardsOver35${suffix}`] || stats.over35Cards || 0) + '%'
      );
      this.updateElement(
        'cardsOver45',
        (stats[`cardsOver45${suffix}`] || stats.over45Cards || 0) + '%'
      );
      this.updateElement(
        'cardsOver55',
        (stats[`cardsOver55${suffix}`] || stats.over55Cards || 0) + '%'
      );
    ...
    updateHalftimeStatistics(stats, filter) {
      ...
      if (filter === 'overall') {
        firstHalfGoalsScored =
          stats.scoredGoalsHT_overall ||
          stats.scored1H_overall ||
          stats.seasonScored1H_overall ||
          0;
        scoredMatchesWithGoals = stats.seasonScored1HMatches_overall || matches;
      } else if (filter === 'home') {
        firstHalfGoalsScored =
          stats.scoredGoalsHT_home || stats.scored1H_home || stats.seasonScored1H_home || 0;
        scoredMatchesWithGoals = stats.seasonScored1HMatches_home || matches;
      } else if (filter === 'away') {
        firstHalfGoalsScored =
          stats.scoredGoalsHT_away || stats.scored1H_away || stats.seasonScored1H_away || 0;
      ...
      const firstHalfGoalsScoredPerc =
        stats[`scored1HPercentage${suffix}`] ||
        stats[`seasonScored1HPercentage${suffix}`] ||
        (scoredMatchesWithGoals > 0
          ? Math.round((firstHalfGoalsScored / scoredMatchesWithGoals) * 100)
          : 0);
      ...
      if (filter === 'overall') {
        firstHalfGoalsConceded =
          stats.concededGoalsHT_overall ||
          stats.conceded1H_overall ||
          stats.seasonConceded1H_overall ||
          0;
      } else if (filter === 'home') {
        firstHalfGoalsConceded =
          stats.concededGoalsHT_home || stats.conceded1H_home || stats.seasonConceded1H_home || 0;
      } else if (filter === 'away') {
        firstHalfGoalsConceded =
          stats.concededGoalsHT_away || stats.conceded1H_away || stats.seasonConceded1H_away || 0;
      }
      ...
      const firstHalfGoalsConcededPerc =
        stats[`conceded1HPercentage${suffix}`] ||
      ...
      if (filter === 'overall') {
        losingAtHT =
          stats.trailingAtHT_overall || stats.losingAtHT_overall || stats.losingAtHT || 0;
      } else if (filter === 'home') {
