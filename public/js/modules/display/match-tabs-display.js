...
export class MatchTabsDisplay {
  ...
  renderH2H(data) {
    ...
    if (!data) {
      if (summaryContainer)
        summaryContainer.innerHTML = '<p class="no-data">H2H data not available</p>';
      if (matchesContainer)
    ...
    if (matchesContainer && data.matches) {
      if (!data.matches.length) {
        ...
        matchesContainer.innerHTML = data.matches
          .map(
            match => `
          <div class="h2h-match">
  ...
  renderLineups(data) {
    ...
    if (!data) {
      if (formationsContainer)
        formationsContainer.innerHTML = '<p class="no-data">Lineups not available yet</p>';
      ...
      if (substitutesContainer)
        substitutesContainer.innerHTML = '<p class="no-data">Substitutes not available yet</p>';
      return;
    }
