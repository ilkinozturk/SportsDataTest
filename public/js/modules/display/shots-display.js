  ...
  class ShotsDisplay {
    ...
    updateShotsStatistics(statistics, filter = 'overall') {
      ...
      if (filter === 'overall') {
        ...
        shotsOnTargetPerMatch = statistics.shotsOnTargetAVG || statistics.shotsOnTargetPerMatch || 0;
        shotsOffTargetPerMatch = statistics.shotsOffTargetAVG || statistics.shotsOffTargetPerMatch || 0;
      } else if (filter === 'home') {
        shotsPerMatch = statistics.shotsPerMatch_home || statistics.homeShotsAVG || 0;
        shotsOnTargetPerMatch = statistics.shotsOnTargetPerMatch_home || statistics.homeShotsOnTargetAVG || 0;
        shotsOffTargetPerMatch = statistics.shotsOffTargetPerMatch_home || statistics.homeShotsOffTargetAVG || 0;
      } else if (filter === 'away') {
        shotsPerMatch = statistics.shotsPerMatch_away || statistics.awayShotsAVG || 0;
        shotsOnTargetPerMatch = statistics.shotsOnTargetPerMatch_away || statistics.awayShotsOnTargetAVG || 0;
        shotsOffTargetPerMatch = statistics.shotsOffTargetPerMatch_away || statistics.awayShotsOffTargetAVG || 0;
      }
      ...
      if (filter === 'overall') {
        ...
        conversionRate = statistics.homeShotsConversionRate || statistics.shotsConversionRate_home || 0;
        shotsPerGoal = statistics.homeShotsPerGoal || statistics.shotsPerGoal_home || 0;
        shotsOnTargetPerGoal = statistics.homeShotsOnTargetPerGoal || statistics.shotsOnTargetPerGoal_home || 0;
      } else if (filter === 'away') {
        conversionRate = statistics.awayShotsConversionRate || statistics.shotsConversionRate_away || 0;
        shotsPerGoal = statistics.awayShotsPerGoal || statistics.shotsPerGoal_away || 0;
        shotsOnTargetPerGoal = statistics.awayShotsOnTargetPerGoal || statistics.shotsOnTargetPerGoal_away || 0;
      }
      ...
      shotThresholds.forEach(threshold => {
        let value;
        if (filter === 'overall') {
          value = statistics[`shotsOver${threshold}`] || statistics[`teamShotsOver${threshold}`] || 0;
        } else if (filter === 'home') {
          value = statistics[`homeShotsOver${threshold}`] || statistics[`teamShotsOver${threshold}_home`] || 0;
        } else if (filter === 'away') {
          value = statistics[`awayShotsOver${threshold}`] || statistics[`teamShotsOver${threshold}_away`] || 0;
        }
      ...
      targetThresholds.forEach(threshold => {
        let value;
        if (filter === 'overall') {
          value = statistics[`shotsOnTargetOver${threshold}`] || statistics[`teamShotsOnTargetOver${threshold}`] || 0;
        } else if (filter === 'home') {
          value = statistics[`homeShotsOnTargetOver${threshold}`] || statistics[`teamShotsOnTargetOver${threshold}_home`] || 0;
        } else if (filter === 'away') {
          value = statistics[`awayShotsOnTargetOver${threshold}`] || statistics[`teamShotsOnTargetOver${threshold}_away`] || 0;
        }
      ...
      matchShotThresholds.forEach(threshold => {
        ...
        if (filter === 'overall') {
          ...
          value = statistics[`homeMatchShotsOver${threshold}`] || statistics[`matchShotsOver${threshold}_home`] || 0;
        } else if (filter === 'away') {
          value = statistics[`awayMatchShotsOver${threshold}`] || statistics[`matchShotsOver${threshold}_away`] || 0;
        }
      ...
      matchTargetThresholds.forEach(threshold => {
        ...
        if (filter === 'overall') {
          ...
          value = statistics[`homeMatchShotsOnTargetOver${threshold}`] || statistics[`matchShotsOnTargetOver${threshold}_home`] || 0;
        } else if (filter === 'away') {
          value = statistics[`awayMatchShotsOnTargetOver${threshold}`] || statistics[`matchShotsOnTargetOver${threshold}_away`] || 0;
        }
    ...
    updateOffsidesStats(statistics, filter) {
      ...
      if (filter === 'overall') {
        ...
        teamOffsidesAvg = statistics.offsidesAvg || statistics.teamOffsidesAvg || statistics.teamOffsidesAVG || 0;
      } else if (filter === 'home') {
        matchOffsidesAvg = statistics.homeMatchOffsidesAvg || statistics.homeMatchOffsidesAVG || 0;
        teamOffsidesAvg = statistics.homeOffsidesAvg || statistics.homeTeamOffsidesAvg || statistics.homeTeamOffsidesAVG || 0;
      } else if (filter === 'away') {
        matchOffsidesAvg = statistics.awayMatchOffsidesAvg || statistics.awayMatchOffsidesAVG || 0;
        teamOffsidesAvg = statistics.awayOffsidesAvg || statistics.awayTeamOffsidesAvg || statistics.awayTeamOffsidesAVG || 0;
      }
      ...
      offsidesThresholds.forEach(threshold => {
        let value = 0;
        if (filter === 'overall') {
          value = statistics[`matchOffsidesOver${threshold}`] || statistics[`offsidesOver${threshold}`] || 0;
        } else if (filter === 'home') {
          value = statistics[`homeMatchOffsidesOver${threshold}`] || statistics[`homeOffsidesOver${threshold}`] || 0;
        } else if (filter === 'away') {
          value = statistics[`awayMatchOffsidesOver${threshold}`] || statistics[`awayOffsidesOver${threshold}`] || 0;
        }
      ...
      ['0_5', '1_5', '2_5'].forEach(threshold => {
        let value = 0;
        if (filter === 'overall') {
          value = statistics[`teamOffsidesOver${threshold}`] || statistics[`offsidesOver${threshold}`] || 0;
        } else if (filter === 'home') {
          value = statistics[`homeTeamOffsidesOver${threshold}`] || statistics[`homeOffsidesOver${threshold}`] || 0;
        } else if (filter === 'away') {
          value = statistics[`awayTeamOffsidesOver${threshold}`] || statistics[`awayOffsidesOver${threshold}`] || 0;
        }
