  ...
  class MatchDetailsManager {
    ...
    renderMatchScore() {
      ...
      if (date) {
        ...
        document.getElementById('matchDateDisplay').textContent = matchDate.toLocaleDateString('tr-TR', {
          weekday: 'long',
          year: 'numeric',
          month: 'long',
          day: 'numeric'
        });
        document.getElementById('matchTimeDisplay').textContent = matchDate.toLocaleTimeString('tr-TR', {
          hour: '2-digit',
          minute: '2-digit'
        });
      }
      ...
      if (homeTeam.logo) {
        ...
        homeLogoEl.src = homeTeam.logo.startsWith('http') ? homeTeam.logo : `https://cdn.footystats.org/img/${homeTeam.logo}`;
      ...
      if (awayTeam.logo) {
        // Fix logo path - add full URL
        awayLogoEl.src = awayTeam.logo.startsWith('http') ? awayTeam.logo : `https://cdn.footystats.org/img/${awayTeam.logo}`;
    ...
    renderKeyStats() {
      ...
      container.innerHTML = keyStats.map(stat => `
        <div class="key-stat">
    ...
    renderH2H() {
      if (!this.matchData) {
        document.getElementById('h2hSummary').innerHTML = '<p class="no-data">Loading match data...</p>';
        document.getElementById('h2hMatches').innerHTML = '<p class="no-data">Loading match data...</p>';
        return;
      }
      ...
      if (!h2h) {
        document.getElementById('h2hSummary').innerHTML = '<p class="no-data">H2H data not available</p>';
        document.getElementById('h2hMatches').innerHTML = '<p class="no-data">No previous meetings found</p>';
        return;
      }
    ...
    renderLineups() {
      if (!this.matchData) {
        document.getElementById('formations').innerHTML = '<p class="no-data">Loading match data...</p>';
        document.getElementById('startingLineups').innerHTML = '<p class="no-data">Loading match data...</p>';
        document.getElementById('substitutes').innerHTML = '<p class="no-data">Loading match data...</p>';
        return;
      }
      ...
      if (!lineups) {
        document.getElementById('formations').innerHTML = '<p class="no-data">Lineups not available yet</p>';
        document.getElementById('startingLineups').innerHTML = '<p class="no-data">Starting XI will be announced 1 hour before kickoff</p>';
        document.getElementById('substitutes').innerHTML = '<p class="no-data">Substitutes not available yet</p>';
        return;
      }
