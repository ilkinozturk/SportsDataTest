  ...
  class DOMHelpers {
      ...
      mutations.forEach(mutation => {
        if (mutation.type === 'childList' || mutation.type === 'attributes') {
          ...
          this.elementCache.forEach((value, key) => {
            if (value.element === mutation.target || mutation.target.contains(value.element)) {
    ...
    getMetrics() {
      const avgOperationTime =
        this.metrics.batchOperations > 0
          ? this.metrics.totalOperationTime / this.metrics.batchOperations
          : 0;

      const cacheHitRate =
        this.metrics.queries > 0 ? (this.metrics.cacheHits / this.metrics.queries) * 100 : 0;
